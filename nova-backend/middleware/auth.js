...
const get_user_from_jwt_token = (token) => {
    ...
    try{
        ...
        const jwt_payload = jwt.verify(token.split("Bearer ")[1], process.env.JWT_SECRET); //if user inactive fro 10+ mins, expire
...
async function check_policy(pwd){
  ...
  const data = await dbAccess.getFilteredAndSortedCollection(SETTINGS_COLLECTION, [SETTINGS_CATEGORY], [SETTINGS_PASSWORD_POLICIES], [0])
...
module.exports = {basic_auth, get_user_from_jwt_token, get_user_from_jwt_token_reset_pwd, check_policy};